...
from plt_optimizer.generate.layout import (
    ...
    _reorder_labels_by_scan,
    _resolve_plate_layouts,
    _transpose_entries,
    generate_layout,
...
class TestRotationDemoExample:
    ...
    def test_example_aborts_without_rotation(self) -> None:
        ...
        Pinned to the ``rows`` frame: the fixture's "rotation is necessary"
        property is specific to row-major packing. In the transposed
        (``columns``) frame the 11"-tall banners stand upright on the 16"
        sheet_2, so the same job fits without rotating (see
        :meth:`test_example_fits_without_rotation_in_columns`).
        """
        labels, job = self._load()
        with pytest.raises(LayoutFitError, match="Could only fit"):
            generate_layout(labels, job.plates, allow_rotation=False, layout=LayoutMode.ROWS)

    def test_example_fits_without_rotation_in_columns(self) -> None:
        """The transposed frame fits the demo job with every label upright.

        Column-major packing offers the packer the plate's height as its
        fill axis, so an 11"-tall banner fits standing on the 16"-tall
        sheet_2 -- a fit the row-major frame only reaches by rotating.
        """
        labels, job = self._load()
        plates = generate_layout(
            labels, job.plates, allow_rotation=False, layout=LayoutMode.COLUMNS
        )
        packed = [pl for p in plates for pl in p.labels]
        assert len(packed) == 14
        assert not any(pl.rotated for pl in packed)


class TestColumnMajorLayout:
    """Tests for the ``layout`` fill-order mode (columns vs. rows)."""
    ...
    def _plate(
        ...
        """Helper to create a zero-margin PlateSpec."""
        return PlateSpec(
            id=plate_id,
            width=width,
            height=height,
    ...
    def test_default_layout_mode_is_columns(self) -> None:
        """The job-level default fill order must be column-major."""
        assert DEFAULT_LAYOUT_MODE == LayoutMode.COLUMNS

    def test_columns_fill_height_before_width(self) -> None:
        """Twelve 6x1 labels on 24x16 must tile column-first.

        The first floor(16/1) = 4 labels share x == 0 with ascending y,
        then the sequence advances rightward -- the inverse of the
        historical row-major order.
        """
        labels = [_make_label(label_id=f"l{i}", width=6.0, height=1.0) for i in range(12)]
        plates = generate_layout(
            labels, [self._plate()], allow_rotation=False, layout=LayoutMode.COLUMNS
        )
        seq = [(pl.label_id, pl.x, pl.y) for pl in plates[0].labels]
        assert seq == [
            ("l0_0", 0.0, 0.0),
            ("l1_0", 0.0, 1.0),
            ("l2_0", 0.0, 2.0),
    ...
    def test_rows_keeps_historical_row_major_order(self) -> None:
        """ROWS mode must reproduce the pre-layout-field coordinates."""
        labels = [_make_label(label_id=f"l{i}", width=6.0, height=1.0) for i in range(12)]
        plates = generate_layout(
            labels, [self._plate()], allow_rotation=False, layout=LayoutMode.ROWS
        )
        seq = [(pl.label_id, pl.x, pl.y) for pl in plates[0].labels]
        assert seq == [
            ("l0_0", 0.0, 0.0),
    ...
    def test_default_layout_argument_matches_columns(self) -> None:
        """Omitting the layout argument must behave like explicit COLUMNS."""
        labels = [_make_label(label_id=f"l{i}", width=6.0, height=1.0) for i in range(12)]
        default = generate_layout(labels, [self._plate()], allow_rotation=False)
        columns = generate_layout(
            labels, [self._plate()], allow_rotation=False, layout=LayoutMode.COLUMNS
        )
        assert default[0].labels == columns[0].labels

    def test_columns_sequence_reads_in_scan_order(self) -> None:
        """``plate.labels`` must be sorted (x, y) under COLUMNS."""
        labels = [_make_label(label_id=f"l{i}", width=6.0, height=1.0) for i in range(12)]
        plates = generate_layout(
            labels, [self._plate()], allow_rotation=False, layout=LayoutMode.COLUMNS
        )
        keys = [(pl.x, pl.y) for pl in plates[0].labels]
        assert keys == sorted(keys)

    def test_columns_preserve_slot_set_and_footprint(self) -> None:
        """Scan reassignment moves identities, never geometry.

        A mixed-size job must produce the identical multiset of
        ``(x, y, w, h)`` slots and the same footprint under both modes:
        the fill order is a permutation within identical footprints only.
        """
        labels = [_make_label(label_id=f"w{i}", width=6.0, height=1.0) for i in range(12)]
        labels += [_make_label(label_id=f"t{i}", width=3.0, height=2.0) for i in range(5)]
        slots = {}
        for mode in (LayoutMode.COLUMNS, LayoutMode.ROWS):
            plates = generate_layout(labels, [self._plate()], allow_rotation=True, layout=mode)
            assert len(plates) == 1
            slots[mode] = sorted(
                (pl.label_id.split("_")[0][0], pl.x, pl.y, pl.width, pl.height)
                for pl in plates[0].labels
            )
            footprint = max(pl.x + pl.width for pl in plates[0].labels) * max(
                pl.y + pl.height for pl in plates[0].labels
            )
            assert footprint == pytest.approx(120.0)
        # Same set of physical slots (geometry multiset), ignoring identities.
        assert sorted(s[1:] for s in slots[LayoutMode.COLUMNS]) == sorted(
            s[1:] for s in slots[LayoutMode.ROWS]
        )

    def test_columns_declaration_order_wins_within_footprint_group(self) -> None:
        """Instances fill scan-order slots in declaration order.

        Two counted labels of identical footprint must appear as
        ``a_0..a_5`` first in scan order, then ``b_0..b_5`` -- proving the
        reassignment follows declaration order rather than id sorting.
        """
        a = _make_label(label_id="a", width=6.0, height=1.0, count=6)
        b = _make_label(label_id="b", width=6.0, height=1.0, count=6)
        plates = generate_layout([a, b], [self._plate()], allow_rotation=False)
        seq = [(pl.label_id, pl.x, pl.y) for pl in plates[0].labels]
        assert seq == [
            ("a_0", 0.0, 0.0),
            ("a_1", 0.0, 1.0),
    ...
    def test_rotation_detection_under_columns(self) -> None:
        """1x3.5 labels on a 12x3 plate rotate; dims swap under COLUMNS."""
        labels = [_make_label(label_id=f"l{i}", width=1.0, height=3.5) for i in range(3)]
        plate = self._plate(plate_id="p2", width=12.0, height=3.0)
        for mode in (LayoutMode.COLUMNS, LayoutMode.ROWS):
            plates = generate_layout(labels, [plate], allow_rotation=True, layout=mode)
            packed = plates[0].labels
            assert len(packed) == 3
            assert all(pl.rotated for pl in packed)
            assert all(pl.width == pytest.approx(3.5) for pl in packed)
            assert all(pl.height == pytest.approx(1.0) for pl in packed)

    def test_mixed_plate_modes_pack_sequentially(self) -> None:
        """Consecutive same-mode plates share a pass; leftovers cascade.

        30 3x6 labels onto a COLUMNS plate then a ROWS plate (16 each):
        plate A receives the first 16 declarations in column-major scan
        order, plate B receives the remaining 14.
        """
        labels = [_make_label(label_id=f"l{i}", width=3.0, height=6.0) for i in range(30)]
        plates = generate_layout(
            labels,
            [
                self._plate(plate_id="A", layout=LayoutMode.COLUMNS),
                self._plate(plate_id="B", layout=LayoutMode.ROWS),
            ],
        )
        assert [p.plate_id for p in plates] == ["A", "B"]
        assert [pl.label_id for pl in plates[0].labels] == [f"l{i}_0" for i in range(16)]
        # Plate A reads column-first: two per column, x ascending.
        assert [(pl.x, pl.y) for pl in plates[0].labels] == [
            (col, row) for col in (0.0, 3.0, 6.0, 9.0, 12.0, 15.0, 18.0, 21.0) for row in (0.0, 6.0)
        ]
        remaining = {pl.label_id for pl in plates[1].labels}
        assert remaining == {f"l{i}_0" for i in range(16, 30)}

    def test_mixed_plate_modes_overflow_raises_constrained_error(self) -> None:
        """Leftovers that outlast every group use the constrained wording."""
        labels = [_make_label(label_id=f"l{i}", width=6.0, height=1.0) for i in range(12)]
        with pytest.raises(LayoutFitError, match="provided plates"):
            generate_layout(
                labels,
                [
                    self._plate(plate_id="small", width=12.0, height=3.0),
                    self._plate(plate_id="tiny", width=6.0, height=1.0, layout=LayoutMode.ROWS),
                ],
            )

    def test_is_transposed(self) -> None:
        """COLUMNS (and raw strings) transpose; ROWS does not."""
        assert _is_transposed(LayoutMode.COLUMNS) is True
        assert _is_transposed(LayoutMode.ROWS) is False
        # str-Enum: raw YAML strings compare by equality, not identity.
        assert _is_transposed(LayoutMode("columns")) is True  # type: ignore[call-arg]
        assert _is_transposed(LayoutMode("rows")) is False  # type: ignore[call-arg]

    def test_transpose_entries_swaps_dims_and_rid_width(self) -> None:
        """Transposed entries carry packer-space dims and rid width."""
        label = _make_label(label_id="a")
        entries = [(6.0, 1.0, ("a_0", label, 6.0))]
        transposed = _transpose_entries(entries, transpose=True)
        assert transposed == [(1.0, 6.0, ("a_0", label, 1.0))]
        assert _transpose_entries(entries, transpose=False) is entries

    def test_resolve_plate_layouts_unbounded_single_group(self) -> None:
        """Unbounded mode yields one default-plate group in the job mode."""
        groups = _resolve_plate_layouts(None, LayoutMode.ROWS, max_default_plates=3)
        assert len(groups) == 1
        assert groups[0].layout == LayoutMode.ROWS
        assert groups[0].bins == (
            ("default_plate_1", DEFAULT_PLATE_WIDTH, DEFAULT_PLATE_HEIGHT),
            ("default_plate_2", DEFAULT_PLATE_WIDTH, DEFAULT_PLATE_HEIGHT),
            ("default_plate_3", DEFAULT_PLATE_WIDTH, DEFAULT_PLATE_HEIGHT),
        )

    def test_resolve_plate_layouts_merges_and_splits_runs(self) -> None:
        """Consecutive same-mode plates merge; mode changes split groups."""
        plates = [
            self._plate(plate_id="a"),  # inherits job COLUMNS
            self._plate(plate_id="b", layout=LayoutMode.COLUMNS),
            self._plate(plate_id="c", layout=LayoutMode.ROWS),
            self._plate(plate_id="d"),  # inherits job COLUMNS -> new group
        ]
        groups = _resolve_plate_layouts(plates, LayoutMode.COLUMNS, max_default_plates=0)
        assert [(g.layout, [b[0] for b in g.bins]) for g in groups] == [
            (LayoutMode.COLUMNS, ["a", "b"]),
            (LayoutMode.ROWS, ["c"]),
            (LayoutMode.COLUMNS, ["d"]),
        ]

    def test_reorder_labels_by_scan_permutes_within_signature(self) -> None:
        """Slot geometry stays; identities move to scan-order slots."""
        label = _make_label(label_id="a")
        slots = [
            PackedLabel(
                label_id="a_1",
        ...
        plate = PackedPlate(plate_id="p", width=24.0, height=16.0, labels=list(slots))
        declaration = {"a_0": 0, "a_1": 1, "a_2": 2}
        _reorder_labels_by_scan([plate], declaration)
        assert [(pl.label_id, pl.x, pl.y) for pl in plate.labels] == [
            ("a_0", 0.0, 0.0),
            ("a_1", 0.0, 2.0),
            ("a_2", 6.0, 0.0),
        ]

    def test_reorder_labels_by_scan_keeps_rotated_groups_separate(self) -> None:
        """Rotated and upright slots never exchange identities."""
        label = _make_label(label_id="a")
        upright = PackedLabel(
            label_id="a_0", x=0.0, y=0.0, width=1.0, height=2.0, rotated=False, source_label=label
        )
        turned = PackedLabel(
            label_id="a_1", x=1.0, y=0.0, width=2.0, height=1.0, rotated=True, source_label=label
        )
        plate = PackedPlate(plate_id="p", width=24.0, height=16.0, labels=[turned, upright])
        _reorder_labels_by_scan([plate], {"a_0": 0, "a_1": 1})
        assert [(pl.label_id, pl.rotated) for pl in plate.labels] == [
            ("a_0", False),
            ("a_1", True),
        ]
