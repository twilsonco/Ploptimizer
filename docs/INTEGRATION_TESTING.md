...
```bash
python run_integration_test.py examples/complex_test_job.yaml
python run_integration_test.py examples/replacement_job.yaml
python run_integration_test.py examples/rotation_demo_job.yaml
```
...
longer for `examples/complex_test_job.yaml`, which renders many labels across
two plates.
...
| `examples/test123_spec.yaml` | Default job spec (reproduces `examples/test123.plt`) |
| `examples/complex_test_job.yaml` | Stress-test spec exercising every pipeline feature |
| `examples/replacement_job.yaml` + `examples/replacement_text_*.txt` | Replacement text file ("badge") template flow |
| `examples/rotation_demo_job.yaml` | Tight 24×10 scrap sheet forcing 90° bin-packing rotation |
...
### `examples/test123_spec.yaml` — Default Job Spec
...
### `examples/complex_test_job.yaml` — Stress-Test Spec
...
the job spec path (default `examples/test123_spec.yaml`, or the CLI override).
...
`examples/rotation_demo_job.yaml` intentionally aborts this way when rotation
is disabled.
...
uv run plt-optimizer generate examples/test123_spec.yaml -o out/ --no-plots
